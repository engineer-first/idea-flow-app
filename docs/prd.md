...
| どの案を選ぶべきか分からない   | 案の良し悪しを話し合う基準が曖昧だと、なんとなくの好みや発言力で決まりやすい。                               | 各決定ステップで「🔴主観ドット×1 + 🔵客観ドット×3」のステルス投票を実施し、主観的な納得感と客観的な比較の両方から絞り込めるようにする。（検討中：得票集計時に主観票を客観票より重く計算する案（例：主観5ポイント・客観1ポイント）も検討されているが未決定）        |
...
- **投票結果が同率の場合は最大2件まで残す**：課題・HMW・アイデアいずれの決定ステップでも、最多得票の付箋が複数ある場合は無理に1件へ絞り込まず、最大2件まで残す。主観ドット・客観ドットの得票数がともに同率の場合は、主観ドットの得票数が多い方を優先して残す。
...
- **ホワイトボードの操作方式はFigJam方式を採用する方向とする**：要素の選択・移動とキャンバスの移動は、V/Hのショートカットキーで切り替えるFigJam方式を採用する方向とする。モバイル利用時の拡張性を考慮した判断とする。
- **タイマーは個人ワークステップで必須、共有ステップ等では必須としない**：各フェーズの個人ワーク（マイ付箋書き出し）ステップではタイマーを必須とする一方、共有・分類などのステップではタイマーを必須にしない。
- **カーソルに参加者名を表示し、共同作業感を演出する**：他メンバーのカーソル位置には表示名を表示する。ただし、ステルス投票中のカーソル表示の扱いは別途設計が必要（検討中）。
- （検討中）共有ステップとグルーピングステップを分割するかどうかは未確定：現在は各フェーズで「共有・分類」を1つのステップとしているが、共有とグルーピングを別ステップに分ける案も検討されている。
- （検討中）アイデア発想（Sketch）フェーズにおけるグルーピングと2軸マッピングの扱い：共有・分類ステップでグルーピングをオフにし、価値×実現難易度の2軸マッピングをより早い段階から用いる案が検討されている。
- （検討中）ステルス投票中の付箋移動可否、投票結果のグループ表示方法など、投票まわりのUI詳細仕様は未確定。
- （検討中）ホワイトボードの全画面表示化、フェーズ・ステップ表示や招待URL表示の開閉式UI化、無限キャンバス化：モックをもとに検討が進められているが、実装時期・詳細仕様は未確定。
...
| ステルス投票（主観×1・客観×3）     | 課題・HMW・アイデアの決定ステップで、他者の投票が見えない形の投票により絞り込める。投票対象はグループではなく常に個々の付箋とする。最多得票が同率の場合は主観票の多い方を優先しつつ最大2件まで残す |
