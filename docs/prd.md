...
| どの案を選ぶべきか分からない   | 案の良し悪しを話し合う基準が曖昧だと、なんとなくの好みや発言力で決まりやすい。                               | 各決定ステップで「🔴主観ドット×1 + 🔵客観ドット×3」のステルス投票を実施し、主観的な納得感と客観的な比較の両方から絞り込めるようにする（主観5ポイント・客観1ポイントで計算）。投票終了後の最終確定は、投票結果画面でホストがカードを選択して行う。               |
...
- **各フェーズの成果物決定は、投票結果画面でホストがカードを選択して確定する**：課題・HMW・アイデアの決定ステップでは、ステルス投票の集計結果を表示した画面上でホストが最終的に採用するカードを選択し、確定操作を行う。投票の集計はあくまで判断材料であり、最終的な確定権限はホスト側に置く。
...
- **投票結果はランキング形式のダイアログで開閉式に表示する**：ステルス投票の集計結果は、主観5ポイント・客観1ポイントで計算したランキング形式のダイアログで表示する。画面を圧迫しない大きさとし、常時表示ではなく開閉式（表示・非表示を切り替え可能）とする。
...
| Step5 課題決定   | 投票結果表示 / ホストが投票結果画面でカードを選び、取り組む課題を確定 | 「取り組む課題が決まりました！ 次は、この課題を『アイデアが生まれやすい問い（HMW）』へ変換していきます。」 |
...
| Step4 HMW決定 | 投票結果表示 / ホストが投票結果画面でカードを選びHMWを確定                                                                                                                                                                                                                                                    | 「HMWが決まりました！ 次は、この問いをもとに解決策を考えていきます。」          |
...
| Step5 集計確認・絞り込み          | 投票結果表示 / ホストが投票結果画面でカードを選び採用アイデアを確定                                                              | 「🎉 採用するアイデアが決まりました！ デザインスプリントはこれで完了です。おつかれさまでした。」                |
...
| ステルス投票（主観×1・客観×3）     | 課題・HMW・アイデアの決定ステップで、他者の投票が見えない形の投票により絞り込める。投票対象はグループではなく常に個々の付箋とする。最終的な確定操作は、投票結果画面でホストが対象カードを選択して行う。 |
