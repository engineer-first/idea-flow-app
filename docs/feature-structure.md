...
Atomic のディレクトリでは 1 つの機能変更が molecules / organisms / templates を
横断し、逆に 1 つのディレクトリには互いに無関係なドメインが「粒度が似ているだけ」
で同居する。パッケージング原則で言う共通閉鎖原則（一緒に変更されるものを
...
- 依存・包含関係は **DAG**（`note-card → sticky-note → note-shadow`）であり、
  ディレクトリは**木**なので、原理的に表現できない。
