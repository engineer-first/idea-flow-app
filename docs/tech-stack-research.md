...
>
> **追記 (2026-07-08)**: R3 の「DevTools で他人の未公開データが見えてはならない」という制約を撤廃し、「UI 上で表示しなければ十分（DevTools 等での技術的な観測は許容する）」に緩和した。R3 に関する記述（§1, §3, §4）はこれに合わせて更新済み。なお本文書の結論（Supabase 継続）自体は、その後の技術再評価と Cloudflare Workers / Durable Objects / D1 への移行（[`docs/refactor-cloudflare-do.md`](./refactor-cloudflare-do.md)）により、R3 とは別の理由（DO のステートマシン権威性）で既に上書きされている。
...
| R3 | 可視性制御 | 個人ワーク中は自分の付箋のみ見える。ステルス投票中は自分の票のみ見える。開票後に集計を公開する。UI 上で他人の未公開データを表示しなければ十分（DevTools 等で技術的に観測可能であることは許容する）。 |
...
| R3 可視性制御 | ○ RLS で宣言的に強制可能。「自分の票しか SELECT できない」を DB 層で保証できる | ○ DO/API のアプリコードでの絞り込みで足りる。UI に表示しなければよく、DB 層での厳密な強制までは不要 | DevTools 等での観測を許容する前提では、両構成とも実装コストは小さく、差は縮む |
...
1. **（2026-07-08 時点で影響縮小）可視性制御の実装コストは、RLS でもアプリコードでも大差なくなった。** ステルス投票・個人ワークは「人によって見えるものが違う」機能だが、DevTools 等での技術的な観測を許容する前提では、UI 表示時にフィルタすれば足りる。Postgres の RLS は宣言的で安全という利点が引き続きあるが、Cloudflare 構成のアプリコードでのフィルタ漏れも「UI に表示されなければ許容」の基準では致命的事故ではなくなり、この観点での A の優位性は縮小している。
