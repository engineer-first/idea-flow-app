...
個人でGoogle Cloud Consoleにアクセスして以下の手順で各自の環境を作成してください
...
1. **プロジェクトの作成 / 選択**
   ...
   - 画面左上のプロジェクト選択プルダウンから、新しいプロジェクトを作成してください。

2. **OAuth同意画面（Consent Screen）の設定**
   - 左側メニューから「APIとサービス」 > 「OAuth同意画面」に移動します。
   - Google Auth Platformの「開始」を押します。
   - 「アプリ情報」など適当に進めて「対象(User Type)」で **外部（External）** を選択し、適当に進めて「作成」ボタンを押します。

1. **OAuthクライアントIDの作成**
   - 作成後の画面に「OAuthクライアント」ボタンが表示されている場合は、それを押します。
   - 表示されていない場合は、左側メニューから「APIとサービス」 > 「認証情報」に移動し、画面上部の「+ 認証情報を作成」をクリックして「OAuth クライアント ID」を選択します。
...
2. **クライアントIDとクライアントシークレットの取得**
   - 作成完了後に表示されるダイアログ、または作成した認証情報の編集画面から、**クライアント ID** を控えておきます。
   - 「OK」を押し、作成したOAuthクライアントの開きます。
   - 画面をスクロールして右のサイドバーから **クライアント シークレット** を入手します。
